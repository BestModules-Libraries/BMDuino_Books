/*
 * =============================================================================
 * 檔案名稱: inactivateRFID_inrfidlist_BMduino.ino
 * 程式語言: Arduino C/C++
 * 檔案類型: 主程式檔（Sketch File）
 * 功能說明: 門禁裝置讀取卡號傳送雲端停用程式（卡片權限撤銷）
 * 
 * =============================================================================
 * 程式運作流程完整解說
 * =============================================================================
 ...
 *   並透過 HTTP GET 請求將卡片號碼傳送至雲端伺服器進行停用（撤銷授權）作業。
 *   當感應到已授權的卡片時，系統會將該卡片的啟用狀態從「已啟用」改為「未啟用」，
 *   使其無法再開啟門鎖。
 * 
 * 【主要功能】
 ...
 *   - 停用程式：將卡片的 activate 狀態設為 0（停用），僅對已存在的卡片生效
 * 
 * 【通訊協定說明】
 ...
 * 
 * 【API 請求範例】
 *   http://iot.arduino.org.tw:8888/bmduino/rfid/disablerfid.php?MAC=483FDACA32A3&KEY=1111222233
 * 
 ...
 * 
 *   2. 卡片已停用（從已啟用狀態變更為停用）：
 *      {"Device": "B8D61A68DAFC", "Card": "1122334455", "Result": "Card Inactivated"}
 ...
 *   5. 本程式僅停用已存在的卡片，不會自動新增或刪除卡片記錄
 *   6. 停用後的卡片將無法通過門禁驗證，直到再次被啟用
 * 
 * 版本: 1.0
 * 建立日期: 2026-03-30
 * =============================================================================
...
// 門禁裝置讀取卡號傳送雲端停用程式 inactivateRFID_inrfidlist_BMduino.ino
