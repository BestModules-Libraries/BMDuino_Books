 ...
 * 功能說明: 雲端門禁系統之通訊模組，負責 HTTP 請求與 JSON 資料解析
 * 
 * =============================================================================
 * 程式運作流程完整解說
 * =============================================================================
 ...
 *   6. 將結果顯示於序列監控視窗（可擴充 OLED 顯示）
 * 
 * 【雲端伺服器規格】
 *   - 伺服器網址：http://iot.arduino.org.tw
 *   - 通訊埠：8888
 *   - API 路徑：/bmduino/rfid/enablerfid.php
 *   - 請求參數：
 *        MAC：設備的 MAC 位址（唯一識別碼）
 *        KEY：讀取到的 RFID 卡號
 *   - 請求範例：
 *        http://iot.arduino.org.tw:8888/bmduino/rfid/enablerfid.php?MAC=483FDACA32A3&KEY=0079262864
 * 
 * 【JSON 回應格式說明】
 ...
 *      說明：卡片已成功啟用，現在可以開啟門鎖
 * 
 *   4. 卡片已更新（已啟用狀態更新時間與 IP）：
 *      {
 *          "Device": "B8D61A68DAFC",
 *          "Card": "1122334455",
 *          "Result": "Card Updated"
 *      }
 *      說明：卡片已存在且已啟用，系統時間與 IP 已更新
 * 
 * 【整體運作流程圖】
 * 
 *   ┌─────────────────────────────────────────────────────────────┐
 ...
 *   │   組合結果範例：                                             │
 *   │   "/bmduino/rfid/enablerfid.php?MAC=483FDACA32A3&KEY=...    │
 *   └─────────────────────────────────────────────────────────────┘
 *                                │
 *                                ▼
 *   ┌─────────────────────────────────────────────────────────────┐
 ...
 *   │       輸出：設備未註冊                                       │
 *   │                                                             │
 *   │   if (Result == "Card Updated")                             │
 *   │       輸出：卡片已更新                                       │
 *   │                                                             │
 *   │   if (Result == "Card Activated")                           │
 *   │       輸出：卡片已啟用                                       │
 *   │                                                             │
...
// 完整路徑範例："/bmduino/rfid/enablerfid.php?MAC=483FDACA32A3&KEY=0079262864"
#define dbagent "/bmduino/rfid/enablerfid.php?MAC=%s&KEY=%s"
 ...
 *   應用程式路徑：/bmduino/rfid/enablerfid.php（伺服器上的 PHP 程式，用來接收並處理資料）
...
void SendtoClouding()     
{
  ...
  if (Wifi.getStatus())  // 若 Wi-Fi 連線正常，回傳 true
  {
    ...
    // 情況三：卡片已啟用
    // 當回傳的 Device 與本機 MAC 相符，且 Result 為 "Card Activated" 時
    // 表示此 RFID 卡片已成功啟用，現在可以開啟門鎖
    if (String(device) == MacData && String(result) == "Card Activated") 
    {
      // 輸出卡片已啟用的成功訊息
      Serial.print("RFID Card (");
      Serial.print(uidStr);
      Serial.print(") is Activated \n");
    }
    
    // 情況四：卡片不存在於資料庫
    // 當回傳的 Device 與本機 MAC 相符，且 Result 為 "Card Not Existed" 時
    // 表示此 RFID 卡片尚未在雲端資料庫中註冊
