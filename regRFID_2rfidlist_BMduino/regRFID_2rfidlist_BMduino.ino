/*
 * 檔案名稱：regRFID_2rfidlist_BMduino.ino
 * 程式語言：Arduino C/C++
...
// 註冊RFID卡片號碼到授權卡片列表之系統  regRFID_2rfidlist_BMduino
 ...
 * 功能：初始化 WiFi 無線網路連線
 * 說明：
 *   1. 呼叫 initWiFi() 進行 WiFi 連線，連線資訊（SSID、密碼）應預先設定於 TCP.h 中
 *   2. 連線完成後呼叫 ShowWiFiInformation() 顯示網路相關資訊
 * 
 * 注意事項：
