...
{  
    ...
    if(BMK52.getINT() == 0)
    {
         ...
         Serial.println(CheckKey());     // 讀取按鍵數值並以二進位格式輸出
    }  
}

//------------------
void init16key()  //初始化BMK52T016 模組：通常是 BestModules 公司推出的 I2C 按鍵矩陣模組，支援中斷觸發
{
      BMK52.begin();         // 初始化 BMK52 模組
}  

int readkeyvalue()  //讀取16 key pad值
{
  if (BMK52.getINT() == 0)  //沒有人按鍵，回傳-1
    {
        return BMK52.readKeyValue() ;  //回傳按下按鍵
    }
    else
    {
    return (-1);  //回傳-1
