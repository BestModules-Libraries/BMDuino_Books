 ...
 *
 * 【與其他模組的關係】
 *   - 本檔被 clouding.h（血氧版與紅外線溫度版）引用，
 *     於 SendtoClouding() 中在 Wi-Fi 未連線時呼叫 initWiFi() 重新連線，
 *     並以 GetMAC() 取得裝置識別碼。
 *   - 本檔被主程式 Oxygen_DataCollectorV2.ino 與
 *     BMduino_IRTemp_Collector_button.ino 引用，於 INITtWIFI() 中
