/********************* 程式整體運作解說 *************************/
/*
 * 【檔案名稱】Oxygen_DataCollectorV2.ino
 * 【程式用途】
 *   本程式為一套「血氧／心率資料採集器」的 Arduino 主程式（Sketch），
 *   主要負責整合以下四大功能模組，並透過 Wi-Fi 將量測結果上傳雲端：
 *     1. TCP.h        ：Wi-Fi／TCP 連線功能封裝（含連線、HTTP、狀態檢查）。
 *     2. OledLib.h    ：OLED 128x64 顯示模組控制（清屏、畫圖、印字串）。
 *     3. OxygenLib.h  ：血氧／心率感測器（BMH83M002）控制封裝。
 *     4. clouding.h   ：雲端上傳封裝（HTTP/MQTT，含 SendtoClouding()）。
 *
 * 【整體運作流程】
 *   ┌─────────────┐
 *   │   setup()   │
 *   └──────┬──────┘
 *          │
 *          ├─(1) initAll()：啟動序列埠（9600 bps）並初始化 OLED 顯示器。
 *          │
 *          ├─(2) INITtWIFI()：初始化 Wi-Fi 連線，並取回 SSID、IP、MAC
 *          │       等網路資訊（儲存於全域變數 SSIDData、IPData、MacData）。
 *          │
 *          ├─(3) 開機歡迎畫面：於 OLED 顯示廠牌 LOGO（128x64 全螢幕），
 *          │       停留 3 秒後清除畫面。
 *          │
 *          ├─(4) 顯示網路資訊：將 MAC（第 0 列）與 IP（第 2 列）顯示於 OLED，
 *          │       方便使用者辨識裝置與除錯。
 *          │
 *          ├─(5) initOxygen()：初始化血氧感測器（設定量測模式與間隔）。
 *          │
 *          ├─(6) 延遲 2 秒，預留時間讓使用者將手指放上感測器。
 *          │
 *          └─(7) readOxygen()：啟動第一次量測，並判定後續運作模式
 *                  （Mode = 0 連續模式、Mode = 1 查詢模式）。
 *
 *   ┌─────────────┐
 *   │   loop()    │  ← 主迴圈持續執行
 *   └──────┬──────┘
 ...
 *          └─ 依全域變數 Mode 的值切換行為：
 *               ├─ case 1（查詢模式）：
 *               │     Mode_ask() 向感測器主動索取一次資料封包；
 *               │     若 readok != 0（本輪成功讀到有效資料），
 *               │     則呼叫 SendtoClouding() 將資料上傳雲端。
 *               │
 *               └─ default（連續模式）：
 *                     Mode_continuous_timing() 檢查緩衝區是否有資料，
 *                     若有則讀出並解析（可依需求判斷 readok 後上傳雲端）。
 *
 * 【重要全域變數】
 *   - Mode    ：運作模式（0：連續模式、1：查詢模式），由 readOxygen() 設定。
 *   - readok  ：本輪是否成功讀到有效資料（非 0 表示成功）。
 *   - SSIDData：目前連線的 Wi-Fi 熱點名稱（SSID）。
 *   - IPData  ：由 AP 分配給本機的 IPv4 位址。
 *   - MacData ：本機 MAC 位址（於其它檔案宣告為全域，常作為裝置唯一 ID）。
 *
 * 【硬體周邊】
 *   - OLED 128x64 顯示器（I2C 或 SPI 介面，由 OledLib 封裝）。
 *   - 血氧／心率感測器 BMH83M002（由 OxygenLib 封裝）。
 *   - Wi-Fi 模組（由 TCP.h 封裝）。
 *
 * 【注意事項】
 *   - 序列埠鮑率固定為 9600 bps，PC 端監控視窗須設定一致。
 *   - 進入 loop() 前需先完成 Wi-Fi 連線與血氧模組初始化。
 *   - 連續模式下若需上傳雲端，建議自行加上頻率控制，避免過度頻繁。
 */
/********************* 模組／函式庫引用 *************************/
